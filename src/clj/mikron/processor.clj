...
(defn global-processors [options]
  (for [processor-type (keys (methods common/global-processor*))]
    (common/global-processor processor-type options)))

(defn local-processors [{:keys [schemas] :as options}]
  (for [processor-type (keys (methods common/local-processor*))
        schema-name    (keys schemas)]
    (common/local-processor processor-type schema-name options)))
...
(defn processors* [options]
  (let [options           (validate/validate-options options)
    ...
    `(letfn [~@local-processors ~@global-processors]
