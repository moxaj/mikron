...
(defmulti measure-stat (fn [stat _ _ _] stat))

(defmethod measure-stat :size [_ method schema value]
  (count (pack method schema value)))

(defmethod measure-stat :pack-time [_ method schema value]
  (->> (crit/quick-benchmark (pack method schema value) {})
       :mean
       (first)
       (* 1000 1000)))

(defmethod measure-stat :unpack-time [_ method schema value]
            ...
            [method
             (vec (for [stat stats]
                    (do (println (format "Measuring %s | %s ..."  (name method) (name stat)))
                        (measure-stat stat method schema data))))]))
        (into {}))])
...
(comment
  ...
  (for [[schema value] [[:doubles doubles] [:snapshot snapshot]]
        method         all-methods]
    (let [value' (->> value
                      (serialize method (get-schema method schema))
                      (deserialize method (get-schema method schema)))]
