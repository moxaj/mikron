...
(defn expand-record [[_ {:keys [extends]} record-map :as record] schemas]
  (if (empty? extends)
    ...
    (let [super-records (map #(expand-record (schemas %) schemas)
                             extends)]
      (reduce (fn [[_ {interp-1 :interp} record-map-1]
                   [_ {interp-2 :interp constructor :constructor} record-map-2]]
                (letfn [(expand-option [opt all] (set (if (= :all opt) all opt)))]
                  [:record {:interp      (set/union (expand-option interp-1 (keys record-map-1))
