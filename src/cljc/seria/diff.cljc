...
(defn equality-operator [schema]
  (let [fn-key (get-in *config* [:eq-ops schema])]
...
(defmethod diff :record [schema value-1 value-2]
  (let [[_ {:keys [constructor]} :as schema] (util/expand-record schema (:schemas *config*))
...
(defmethod diff :custom [schema value-1 value-2]
  `(~(util/runtime-processor schema (case *direction*
                                      :diff   :differ
                                      :undiff :undiffer))
